...
import connectionLevelExporterFields from 'src/app/connection-level/list/connection-level-exporter-fields';
...
export class ConnectionLevelListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await ConnectionLevelApi.list(
        filter,
        this.orderBy,
        null,
        null,
