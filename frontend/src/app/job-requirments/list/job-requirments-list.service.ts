...
import jobRequirmentsExporterFields from 'src/app/job-requirments/list/job-requirments-exporter-fields';
...
export class JobRequirmentsListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await JobRequirmentsApi.list(
        filter,
        this.orderBy,
        null,
        null,
