...
export const tenantSubdomain = {
  get isEnabled() {
    return (
      environment.tenantMode === 'multi-with-subdomain'
    );
  },
  ...
  fromLocationHref() {
    if (environment.tenantMode !== 'multi-with-subdomain') {
      return null;
    }
    ...
    const domainDotsCount = environment.frontendUrl.host.split(
      '.',
    ).length;
  ...
  fullTenantUrl(tenantUrl) {
    return `${environment.frontendUrl.protocol}://${tenantUrl}.${environment.frontendUrl.host}`;
  },
