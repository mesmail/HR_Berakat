...
import trainingCertificatesExporterFields from 'src/app/training-certificates/list/training-certificates-exporter-fields';
...
export class TrainingCertificatesListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await TrainingCertificatesApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        trainingCertificatesExporterFields,
