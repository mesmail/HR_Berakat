...
import employmentContractExporterFields from 'src/app/employment-contract/list/employment-contract-exporter-fields';
...
export class EmploymentContractListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await EmploymentContractApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        employmentContractExporterFields,
        'employmentContract',
