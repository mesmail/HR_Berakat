...
import leaveApplicationFormExporterFields from 'src/app/leave-application-form/list/leave-application-form-exporter-fields';
...
export class LeaveApplicationFormListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await LeaveApplicationFormApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        leaveApplicationFormExporterFields,
