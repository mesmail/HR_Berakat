  ...
  providers: [
    {
      ...
      useExisting: DateTimeRangeInputComponent,
    },
  ],
  host: {
    '[class.app-date-time-range-input-floating]':
      'shouldLabelFloat',
    '[id]': 'id',
...
export class DateTimeRangeInputComponent
  implements
    MatFormFieldControl<DateTimeRange>,
    ControlValueAccessor,
    OnDestroy {
  onChange = (value: any) => {};
  ...
  focused = false;
  errorState = false;
  controlType = 'app-date-time-range-input';
  id = `app-date-time-range-input-${DateTimeRangeInputComponent.nextId++}`;
  describedBy = '';
  ...
  set value(range: DateTimeRange | null) {
    const { start, end } =
