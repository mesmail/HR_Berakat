...
import tellProblemExporterFields from 'src/app/tell-problem/list/tell-problem-exporter-fields';
...
export class TellProblemListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await TellProblemApi.list(
        filter,
        this.orderBy,
        null,
        null,
