...
import commonComiteesExporterFields from 'src/app/common-comitees/list/common-comitees-exporter-fields';
...
export class CommonComiteesListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await CommonComiteesApi.list(
        filter,
        this.orderBy,
        null,
        null,
