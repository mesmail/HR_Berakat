...
import usersNewExporterFields from 'src/app/users-new/list/users-new-exporter-fields';
...
export class UsersNewListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await UsersNewApi.list(
        filter,
        this.orderBy,
        null,
        null,
