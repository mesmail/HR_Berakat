...
import artisticSkillsExporterFields from 'src/app/artistic-skills/list/artistic-skills-exporter-fields';
...
export class ArtisticSkillsListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await ArtisticSkillsApi.list(
        filter,
        this.orderBy,
        null,
        null,
