...
import cardInformationExporterFields from 'src/app/card-information/list/card-information-exporter-fields';
...
export class CardInformationListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await CardInformationApi.list(
        filter,
        this.orderBy,
        null,
        null,
