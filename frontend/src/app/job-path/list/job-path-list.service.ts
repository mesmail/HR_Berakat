...
import jobPathExporterFields from 'src/app/job-path/list/job-path-exporter-fields';
...
export class JobPathListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await JobPathApi.list(
        filter,
        this.orderBy,
        null,
        null,
