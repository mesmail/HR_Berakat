...
import jobFrameworksExporterFields from 'src/app/job-frameworks/list/job-frameworks-exporter-fields';
...
export class JobFrameworksListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await JobFrameworksApi.list(
        filter,
        this.orderBy,
        null,
        null,
