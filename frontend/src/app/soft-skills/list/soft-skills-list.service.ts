...
import softSkillsExporterFields from 'src/app/soft-skills/list/soft-skills-exporter-fields';
...
export class SoftSkillsListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await SoftSkillsApi.list(
        filter,
        this.orderBy,
        null,
        null,
