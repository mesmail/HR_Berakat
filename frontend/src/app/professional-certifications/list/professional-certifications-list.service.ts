...
import professionalCertificationsExporterFields from 'src/app/professional-certifications/list/professional-certifications-exporter-fields';
...
export class ProfessionalCertificationsListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await ProfessionalCertificationsApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        professionalCertificationsExporterFields,
