...
import tasksDutiesExporterFields from 'src/app/tasks-duties/list/tasks-duties-exporter-fields';
...
export class TasksDutiesListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await TasksDutiesApi.list(
        filter,
        this.orderBy,
        null,
        null,
