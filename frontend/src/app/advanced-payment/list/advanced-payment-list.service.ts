...
import advancedPaymentExporterFields from 'src/app/advanced-payment/list/advanced-payment-exporter-fields';
...
export class AdvancedPaymentListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await AdvancedPaymentApi.list(
        filter,
        this.orderBy,
        null,
        null,
